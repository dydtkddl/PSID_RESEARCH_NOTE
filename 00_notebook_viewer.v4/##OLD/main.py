...
# ======= stdlib =======
import sys          # <--- 최상단 배치
import asyncio      # <--- 최상단 배치

# ✅ [핵심 수정] 조건문 없이 파일 로드 시 즉시 실행되도록 최상단에 배치
# 이 코드는 다른 import 문이나 app = FastAPI() 보다 무조건 위에 있어야 안전합니다.
if sys.platform == "win32":
    # 윈도우에서는 SelectorEventLoop(기본값)가 Playwright(Subprocess)를 지원하지 않음
    # 따라서 ProactorEventLoop로 강제 교체
...
ROOT_DIR = Path(r"C:\Users\user\Desktop\Reseach_Note").expanduser().resolve()
...
# [수정] 댓글 로드/저장 유틸리티 (하이라이트와 동일한 구조인 Dict[경로, 리스트]로 변경)
def _load_comments_store() -> Dict[str, List[Dict[str, Any]]]:
    with _CMT_LOCK:
        data = load_json_file(COMMENTS_FILE, {})
        # 파일이 비어있거나 리스트 형태라면 딕셔너리로 초기화
        if not isinstance(data, dict):
            return {}
        return data

def _save_comments_store(store: Dict[str, List[Dict[str, Any]]]) -> None:
    with _CMT_LOCK:
        ...
        try:
            ...
            logger.exception("Failed to save comments store: %s", e)
            try:
                if tmp.exists():
                    tmp.unlink()
            except Exception:
...
class CommentAddPayload(BaseModel):
    rel_path: str
    content: str
    parent_id: Optional[str] = None  # 대댓글일 경우 부모 ID
...
def build_pdf_from_markdown_file(
    ...
    """
    - frontmatter 제거 후 Metadata 섹션 삽입
    - 히스토리(.history)도 원본 폴더 기준으로 이미지 resolve
    - 이미지 평탄화(파일명 충돌시 _2 ...) + temp root에서 markdown-pdf 렌더
    - PyMuPDF TOC level 에러 자동 보정
    ...
    with tempfile.TemporaryDirectory(prefix="mdpdf_assets_") as tmpdir:
        ...
        pdf.add_section(
            Section(rewritten_md, root=str(tmpdir_p), toc=True),
            user_css=PDF_USER_CSS,
        )
        ...
        try:
            ...
            if "hierarchy level of item 0 must be 1" in str(e):
                logger.warning("TOC hierarchy error caught. Fixing toc levels and retry...")
...
async def build_pdf_from_markdown_file(
    note_rel_norm: str,
    md_text: str,
    out_pdf_path: Path,
) -> Tuple[int, int]:
    """
    [Playwright 버전]
    - Headless Browser를 사용하여 MathJax 수식을 완벽하게 렌더링한 후 PDF로 저장
    - 기존 이미지 평탄화 로직 유지
    """
    logger.info("PDF Generation started via Playwright for: %s", note_rel_norm)

    # 1) frontmatter 처리 + 수식 < > 보호
    md_for_pdf = build_md_for_pdf(md_text)
    md_for_pdf = sanitize_math_angles(md_for_pdf)

    # 2) HTML 변환 (render_markdown 사용)
    #    - 주의: render_markdown은 <body> 내용만 반환하므로, 아래에서 전체 HTML 뼈대를 만들어야 함
    body_html = render_markdown(md_for_pdf)

    # 3) 히스토리 폴더 등 고려하여 이미지 경로 기준점 잡기
    content_dir_rel = get_content_dir_rel(note_rel_norm)

    # 4) 이미지 평탄화 (기존 로직 재사용 - 중요!)
    #    Playwright가 로컬 파일을 잘 읽을 수 있도록 기존처럼 zip/temp 폴더 구조 활용
    #    다만, render_markdown을 이미 했으므로 HTML 상태에서 이미지 경로만 다시 맞춰줍니다.
    #    (기존 함수 rewrite_markdown_images... 는 MD 대상이므로, 여기서는 HTML 정규화가 필요하나,
    #     가장 쉬운 방법은 Playwright에게 로컬 파일 접근 권한을 주는 것입니다.
    #     하지만 기존 로직 일관성을 위해 이미지 복사 방식을 유지합니다.)
    
    # 4-1) MD 텍스트 단계에서 이미지를 평탄화용 이름으로 치환 (기존 함수 활용을 위해 순서 조정)
    rewritten_md, copies = rewrite_markdown_images_for_flat_bundle(md_for_pdf, content_dir_rel)
    
    # 4-2) 치환된 MD를 다시 HTML로 렌더링
    final_body_html = render_markdown(rewritten_md)

    # 5) 임시 폴더 작업
    with tempfile.TemporaryDirectory(prefix="pdf_gen_") as tmpdir:
        tmpdir_p = Path(tmpdir)
        
        # 이미지 복사 (기존 로직 유지 + 로그)
        ok_imgs = 0
        total_imgs = len(copies)
        
        if total_imgs > 0:
            logger.info("Copying %d images for PDF...", total_imgs)
            for img_rel, arcname in tqdm(copies, desc="PDF Assets", unit="img"):
                try:
                    src = safe_join(img_rel)
                    ...
                    logger.warning("Failed to copy img: %s", e)

        # 6) 완전한 HTML 파일 생성 (MathJax 포함!)
        #    CSS는 base.html과 유사하게, PDF용으로 깔끔하게 정의
        full_html = f"""
        <!DOCTYPE html>
        <html>
        <head>
            ...
            <style>
                ...
                th, td {{ border: 1px solid #ccc; padding: 8px; font-size: 9pt; }}
                th {{ background-color: #f5f5f5; }}
                code {{ font-family: Consolas, monospace; background: #f0f0f0; padding: 2px 4px; border-radius: 3px; }}
                pre {{ background: #f8f8f8; padding: 10px; border-radius: 5px; overflow-x: auto; white-space: pre-wrap; }}
        ...
        temp_html_path = tmpdir_p / "index.html"
        with open(temp_html_path, "w", encoding="utf-8") as f:
            f.write(full_html)

        # 7) Playwright로 PDF 인쇄
        logger.info("Launching Headless Browser for PDF rendering...")
        async with async_playwright() as p:
            ...
            # ✅ 핵심: MathJax 렌더링이 끝날 때까지 대기
            # 수식이 많으면 시간이 걸리므로 안전하게 처리
            try:
                await page.wait_for_selector("mjx-container", timeout=5000) # 수식이 하나라도 있으면 대기
            except Exception:
                # 수식이 없는 문서일 수도 있음
                pass
            
            # 네트워크 유휴 상태(이미지 로딩 완료) 대기
            await page.wait_for_load_state("networkidle")

            # PDF 저장
            await page.pdf(path=str(out_pdf_path), format="A4", margin={"top": "20mm", "bottom": "20mm", "left": "20mm", "right": "20mm"})
            
            await browser.close()
            
        logger.info("PDF saved via Playwright: %s (images %d/%d)", out_pdf_path, ok_imgs, total_imgs)
        return ok_imgs, total_imgs
...
async def download_pdf(rel_path: str) -> FileResponse:
    ...
    try:
        # ok, total = build_pdf_from_markdown_file(rel_norm, md_text, tmp_path)
        # logger.info("PDF convert done: images %d/%d", ok, total)
        ok, total = await build_pdf_from_markdown_file(rel_norm, md_text, tmp_path)
...
async def delete_highlight(payload: HighlightDeletePayload) -> JSONResponse:
    ...
    return JSONResponse({"ok": True})
# [수정] 댓글 API: 대댓글 지원 및 정렬 로직 강화

@app.get("/research/comments", name="list_comments")
async def list_comments(rel_path: str = Query(...)) -> JSONResponse:
    """해당 파일의 댓글 목록 조회"""
    rel_norm = rel_path.replace("\\", "/")
    
    store = _load_comments_store()
    items = store.get(rel_norm, [])
...
@app.post("/research/comments/add", name="add_comment")
async def add_comment(payload: CommentAddPayload) -> JSONResponse:
    """댓글 또는 대댓글 추가"""
    rel_norm = payload.rel_path.replace("\\", "/")
    ...
    item = {
        "id": new_id,
        "parent_id": payload.parent_id, # 대댓글인 경우 부모 ID 저장
        "content": content,
        "created_at": now_str,
        "updated_at": now_str,
    }
    ...
    _save_comments_store(store)
    
    logger.info("Comment added: %s (id=%s, parent=%s)", rel_norm, new_id, payload.parent_id)
    return JSONResponse({"ok": True, "item": item})

@app.post("/research/comments/update", name="update_comment")
async def update_comment(payload: CommentUpdatePayload) -> JSONResponse:
    """댓글 수정"""
    content = payload.content.strip()
    if not content:
        raise HTTPException(status_code=400, detail="내용을 입력해주세요.")
...
@app.post("/research/comments/delete", name="delete_comment")
async def delete_comment(payload: CommentDeletePayload) -> JSONResponse:
    """댓글 삭제 (대댓글이 달린 부모 댓글 삭제 시 대댓글도 함께 삭제)"""
    store = _load_comments_store()
    ...
    if found_path:
        original_items = store[found_path]
        # 타겟 댓글과, 그 댓글을 부모로 가진 대댓글들을 모두 제거
        new_items = [
            i for i in original_items 
            if i.get("id") != target_id and i.get("parent_id") != target_id
        ]
        store[found_path] = new_items
        _save_comments_store(store)
        logger.info("Comment deleted: %s (and its replies)", target_id)
        return JSONResponse({"ok": True})
    else:
        return JSONResponse({"ok": True})
...
if __name__ == "__main__":
    ...
    # 1. 윈도우에서 Playwright 사용 시 필수: ProactorEventLoop 강제 설정
    if sys.platform == "win32":
        asyncio.set_event_loop_policy(asyncio.WindowsProactorEventLoopPolicy())

    # 2. 핵심 수정: loop="asyncio" 옵션 추가
    # 이 옵션이 없으면 uvicorn이 설정을 무시할 수 있습니다.
    uvicorn.run(
        "main:app", 
        host="0.0.0.0", 
        port=8151, 
        reload=False, 
        loop="asyncio"  # <--- ✅ 이 부분이 반드시 있어야 합니다!
    )